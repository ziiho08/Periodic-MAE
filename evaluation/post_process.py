...
def get_hr(y, sr, min=45, max=150):
    p, q = welch(y, sr, nfft=1e5/sr, nperseg=np.min((len(y)-1, 256)))
    return p[(p>min/60)&(p<max/60)][np.argmax(q[(p>min/60)&(p<max/60)])]*60

def get_psd(y, sr, min=45, max=150):
    p, q = welch(y, sr, nfft=1e5/sr, nperseg=np.min((len(y)-1, 256)))
...
def calculate_metric_per_video(predictions, labels, fs, diff_flag=True, use_bandpass=True, hr_method='FFT'):
...
def calculate_hr(predictions, labels, fs, diff_flag=False):
    """Calculate video-level HR and SNR"""
    if diff_flag:  # if the predictions and labels are 1st derivative of PPG signal.
        predictions = _detrend(np.cumsum(predictions), 100)
        labels = _detrend(np.cumsum(labels), 100)
...
def calculate_psd(predictions, labels, fs, diff_flag=False):
    """Calculate video-level HR and SNR"""
    if diff_flag:  # if the predictions and labels are 1st derivative of PPG signal.
        predictions = _detrend(np.cumsum(predictions), 100)
        labels = _detrend(np.cumsum(labels), 100)
...
def numpy_HR_calculation(signal, fps):
    # perform FFT for power spectrum analysis
