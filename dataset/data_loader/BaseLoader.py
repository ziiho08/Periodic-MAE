...
Dataset already supported: UBFCrPPG, PURE, SCAMPS, BP4D+, and UBFC-PHYS.
...
class BaseLoader(Dataset):
    ...
    def __init__(self, dataset_name, raw_data_path, config_data):
        ...
        self.file_list_path = os.path.join(self.cached_path, 'DataFileLists')
        self.preprocessed_data_len = 0
        self.data_format = config_data.DATA_FORMAT
        self.do_preprocess = config_data.DO_PREPROCESS
        self.config_data = config_data
        self.img_size = 128
        self.clip_frames = 160
        self.patch_size = 16
        self.tubelet_size = 1
        self.mask_percentage_target = 1 - config_data.MASK_RATIO # Default 0.7
        self.mask_unit_num = self.img_size // self.patch_size
        self.mask_strategy = "Periodic_frame"
        self.decoder_mask_strategy = "running_cell"
        self.patch_masking = True
        self.current_step = config_data.MASK_RATIO
    ...
    def set_mask_ratio(self, mask_ratio):
        self.current_step = mask_ratio  # Update the step  
        print('Step', self.current_step, end='\n\n')
    ...
    def __getitem__(self, index):
        ...
        masks = torch.zeros(self.clip_frames // self.tubelet_size, self.mask_unit_num, self.mask_unit_num, dtype=torch.float32)  # 80, 8, 8\
        self.mask_percentage_target_for_frame = 1 - self.current_step
        ...
        if self.mask_strategy == "tube": # Source code: https://github.com/ControlNet/MARLIN/
        ...
        elif self.mask_strategy == "Periodic_frame":

            frame_mask = torch.ones(self.clip_frames // self.tubelet_size)
            target_visible_num = ceil(len(frame_mask) * self.mask_percentage_target_for_frame)
            visible_indexes = frame_mask.nonzero().flatten()
            step = ceil(len(visible_indexes) / target_visible_num)
            extra_indexes = visible_indexes[::step]

            non_extra_indexes = torch.arange(len(frame_mask))
            non_extra_indexes = non_extra_indexes[~torch.isin(non_extra_indexes, extra_indexes)]
            frame_mask[non_extra_indexes] = False
            masks = rearrange(frame_mask, "t -> t 1 1").expand_as(masks).flatten().bool()
            speed = 1/step

        elif self.mask_strategy == "random_frame":
            frame_mask = torch.ones(self.clip_frames // self.tubelet_size)
        ...
        elif self.mask_strategy == "running_cell": # Source code: https://github.com/MCG-NJU/VideoMAE
            num_masks_per_cell = int(4 * self.mask_percentage_target)
            assert 0 < num_masks_per_cell < 4
            num_patches_per_cell = 4 - num_masks_per_cell

            cell_size = num_masks_per_cell + num_patches_per_cell
            queue = np.hstack([np.ones(num_masks_per_cell), np.zeros(num_patches_per_cell)])
            queue_ptr = np.random.randint(cell_size)

            mask_list = []
            for _ in range(self.clip_frames // self.tubelet_size):
                cell_idx = (np.arange(cell_size) + queue_ptr) % cell_size
                mask_unit = queue[cell_idx].reshape(2, 2)
                
                mask_map = np.tile(mask_unit, [self.input_height // 2, self.input_width // 2])
                mask_list.append(mask_map.flatten())
                queue_ptr += 1

            masks = np.stack(mask_list, axis=0)
            masks = torch.tensor(masks, dtype=torch.bool).flatten()
        ...
        return data, label, filename, chunk_id, speed, masks.flatten().bool()
    ...
    def face_detection(self, frame, backend, use_larger_box=False, larger_box_coef=1.0):
        ...
        if backend == "HC":
            ...
            if isinstance(detected_faces, tuple):
                res, other_info = detected_faces
            else:
                res = detected_faces  # 딕셔너리만 반환되는 경우
    ...
    def normalized_data(data):
        """Normalization for video data"""
        
        if data.max() > 1:
            data = data / 255.0
            print(data.min(), data.max())
        input_mean = np.array([0.485, 0.456, 0.406])  # R, G, B 채널별 평균
        input_std = np.array([0.229, 0.224, 0.225])   # R, G, B 채널별 표준편차
        for i in range(data.shape[-1]):  # 마지막 차원이 채널(RGB)
            data[..., i] = (data[..., i] - input_mean[i]) / input_std[i]
        data[np.isnan(data)] = 0
        print(data.min(), data.max())
        return data
