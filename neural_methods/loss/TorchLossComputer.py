...
from evaluation.post_process import calculate_hr , calculate_psd, torch_HR_calculation
from unsupervised_methods.methods.POS_WANG import torch_detrending_filter,  torch_butter_bandpass_filter
...
class PeriodicMAE_Loss(nn.Module):
    def __init__(self):
        super(PeriodicMAE_Loss,self).__init__()
        self.criterion_Pearson = Neg_Pearson()
        self.MSE_loss = nn.MSELoss()

    def forward(self, pred_ppg, labels , epoch , FS , diff_flag, device):    

        y_dtd = torch_detrending_filter(labels, device=device, lam=50)
        Filtered_ref = torch_butter_bandpass_filter(y_dtd, 0.5, 3, FS, order=3, device=device)
        ref_HR, _, ref_amplitude, _ = torch_HR_calculation(Filtered_ref, FS, device)
        
        x_dtd = torch_detrending_filter(pred_ppg, device=device, lam=50)
        Filtered_pred_rPPG = torch_butter_bandpass_filter(x_dtd, 0.5, 3, FS, order=3, device=device)
        pred_rPPG_HR, _, pred_rPPG_amplitude, _ = torch_HR_calculation(Filtered_pred_rPPG, FS, device)
        
        freq_loss = self.MSE_loss(ref_amplitude, pred_rPPG_amplitude)
        time_loss = self.criterion_Pearson(pred_ppg.view(1,-1) , labels.view(1,-1))
        HR_loss = self.MSE_loss(pred_rPPG_HR, ref_HR)

        loss = 0.001 * HR_loss + 100 * freq_loss + time_loss
        return loss
